 ...
 *
 * The font files are only provided in response to a
 * browser request, so this script mocks browser that will
 * return WOFF2 format.
 *
 * This script saves those files as
 * well as two configuration files: a JSON file with all
 * of the fonts and a `embed.json` file in each font
 * directory with the @font-face declarations needed to
 * load the font for each subset (eg - latin).
 *
 * The embed.json file references relative URLs to the
 * font paths. These need to be adapted based on where
 * the fonts are being deployed.
 *
 ...
 * Run the script
 *
 * Downloads all font files that have been updated since the
...
const run = async () => {
  ...
  fs.writeFileSync(FONT_LIST_FILE, JSON.stringify(fonts, null, 2))
...
const saveFont = async (font) => {
  ...
  fs.writeFileSync(`${dir}/embed.json`, JSON.stringify(localSubsets, null, 2))
  ...
  const requests = urls.map(async url => {
    ...
    if (!filename) {
      throw new Error(`Unable to extract filename from download URL ${url} for ${font.family}`)
    }
    return downloadFile(url, `${dir}/${filename}`)
  })

  // Get the "menu" font file
  requests.push = downloadFile(font.menu, `${dir}/menu.woff2`)
  ...
  return fetch(url)
    .then(r => {
      if (r.ok && r.body) {
        ...
        Readable.fromWeb(r.body).pipe(stream)
        return true
      }
    })
    .catch(err => {
...
/**
 * Extract the filename of a font file from its download URL
 */
const getFilenameFromUrl = url => {
  const matches = [...url.match(/fonts.gstatic.com\/s\/[^\/]*\/[^\/]*\/(.*)/i)]
  if (matches.length) {
    return matches[1]
  }
  return ''
